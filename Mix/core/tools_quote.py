...
async def sender_name(m):
    if m.forward_date:
        ...
        if m.from_nlx.last_name:
            return f"{m.from_nlx.first_name} {m.from_nlx.last_name or ''}"
        else:
            return m.from_nlx.first_name
    elif m.sender_chat:
...
async def sender_emoji(m):
    ...
    return m.from_nlx.emoji_status.custom_emoji_id if m.from_nlx else ""


async def sender_nlxname(m):
    if m.forward_date:
        if (
            not m.forward_sender_name
            and not m.forward_from
            and m.forward_from_chat
            and m.forward_from_chat.nlxname
        ):
            return m.forward_from_chat.nlxname
        elif (
            not m.forward_sender_name
            and not m.forward_from
            and m.forward_from_chat
...
async def sender_photo(m):
    ...
    elif m.from_nlx and m.from_nlx.photo:
        return {
            "small_file_id": m.from_nlx.photo.small_file_id,
            "small_photo_unique_id": m.from_nlx.photo.small_photo_unique_id,
            "big_file_id": m.from_nlx.photo.big_file_id,
            "big_photo_unique_id": m.from_nlx.photo.big_photo_unique_id,
        }
    elif m.from_nlx or m.sender_chat and not m.sender_chat.photo or not m.sender_chat:
        return ""
    else:
        return {
...
async def quotly(messages, kolor):
    ...
    for m in messages:
        ...
        m_dict["from"]["nlxname"] = await sender_nlxname(m)
