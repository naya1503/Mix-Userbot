...
async def translate_file(file_path, translator, target_lang):
    ...
    for key, value in data.items():
        if isinstance(value, str):
            ...
            translated_value = await translator.translate(value.strip(), target_lang=target_lang)
            translated_data[translated_key] = translated_value
        elif isinstance(value, dict):
            translated_data[key] = await translate_nested(value, translator, target_lang)
        else:
