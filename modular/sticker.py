...
async def _(self: nlx, m):
    ...
    em.initialize()
    rep = m.reply_to_message
    await nlx.unblock_nlx(bot.me.nlxname)
    if not rep:
        await m.reply(cgr("st_3").format(em.gagal))
        return
    if rep.sticker:
        pros = await m.reply(cgr("proses").format(em.proses))
        ai = await nlx.forward_messages(bot.me.nlxname, m.chat.id, message_ids=rep.id)
        await nlx.send_message(bot.me.nlxname, "/unkang", reply_to_message_id=ai.id)
...
async def _(self: nlx, m):
    ...
    if not rep:
        ...
        return
    await nlx.send_message(bot.me.nlxname, "/kang")
    pros = await m.reply(cgr("proses").format(em.proses))
    ai = await nlx.forward_messages(bot.me.nlxname, m.chat.id, message_ids=rep.id)
    await nlx.send_message(bot.me.nlxname, "/kang", reply_to_message_id=ai.id)
    await asyncio.sleep(5)
    async for tai in nlx.search_messages(
        bot.me.nlxname, query="Sticker Anda Berhasil Dibuat!", limit=1
    ):
        await asyncio.sleep(5)
        await tai.copy(m.chat.id)
    await pros.delete()
    ulat = await nlx.resolve_peer(bot.me.nlxname)
    await nlx.invoke(DeleteHistory(peer=ulat, max_id=0, revoke=True))
    return


async def resleting(m):
    return [x async for x in nlx.get_chat_history(bot.me.nlxname, limit=1)][0].text
